...
## 2) Laço de Repetição (Loop) for:
#Esta linha inicia um loop que irá iterar sobre cada elemento da lista lista_glicemias. 
#A cada iteração, a variável item será igual a um dos valores da lista.
for item in lista_glicemias:    

## 3)Condicional if-elif-else:
#Dentro do loop, há uma série de instruções condicionais que verificam o valor de item.
...
## calcular a média dos valores em uma lista em Python. 
...
## calcular a mediana dos valores em uma lista em Python. 
...
## calcular quantas vezes ocorreram episódios de hipoglicemia.
...
## calcula a porcentagem de episódios de hipoglicemia
...
## calcula a moda do conjunto de dados contidos na lista lista_glicemias
valor_moda = ""
qtd_moda = 0
for item in lista_glicemias:
    ocorrencias = 0
